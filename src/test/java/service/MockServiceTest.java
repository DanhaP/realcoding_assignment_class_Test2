...
@RunWith(MockitoJUnitRunner.class) //mocket set up
public class MockServiceTest {
    ...
    public void confirmArraySizeCheck(){
        ...
        System.out.println("Size ::"+mockChampions.size()); //mock객체라서 size가 0
        verify(mockChampions, times(1)).size(); //실패, size를 실행하지 않음
    }


    // 4-1. champion 객체에서 이름을 가져오는 로직이 2번 이상 실행되면 Pass 하는 로직을 작성하세요.
    @Test
    public void shouldTwoTimesInvacationForGetChampionName(){
        Champion champion = mock(Champion.class);
        ...
        verify(champion, atLeast(2)).getName(); //최소 2번 실행 시 통과
    }
    ...
    public void shouldNeverInvocationWhenChampionSetName(){
        ...
        verify(champion, never()).setName("잭스");
    }
    // 4-4. champion 객체에서 이름을 가져오는 로직이 200ms 시간 이내에 1번 실행되었는 지 검증하는 로직을 작성하세요.
    @Test
    public void shouldOneTimeInvocation200msForChampionGetName(){
        Champion champion = mock(Champion.class);
        champion.setName("트페");
        champion.setPosition("미드");
        ...
        System.out.println("챔피언 : "+champion.getName());
        verify(champion,timeout(200).atLeastOnce());
    }
    ...
    @Test
    public void MockMidLaneChampion(){
    ...
    @Test
    public void testInvacationCountWhenGetChampionList(){
        List<Champion> champions = new ArrayList<Champion>();
        champions.add((new Champion("루시안","바텀",5)));
        champions.add((new Champion("유미","서폿",3)));
        champions.add((new Champion("애쉬","바텀",6)));
